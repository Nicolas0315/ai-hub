...
export class SynergyEngine {
  ...
  public logMetrics(result: SynergyResult): void {
    console.log(`[SynergyEngine] [${result.metadata.timestamp}] Synergy Score: ${result.combinedScore.toFixed(2)}`);
    ...
    console.log(` └ System Confidence: ${(result.dimensions.confidenceAdjustment * 100).toFixed(0)}%`);
  }
}
