# OpenClaw突合仕様書: KQ Mandatory Hard-Bind

更新日時: 2026-03-07 JST  
対象: `https://github.com/openclaw/openclaw` の実装系に対する Enigma/KQ 強制通過仕様

---

## 1. 目的

通常チャット受信から応答送信まで、**全経路で KQ パケットを必須化**し、
`AgentClaw -> inf-Coding -> inf-Bridge -> KQ -> inf-Brain` をバイパス不能にする。
...
- 受信時に `KQ_INPUT_PACKET_JSON` を生成し、contextへ添付すること
- パケット欠落/不正時は **fail-close**（応答生成しない）

### 4.2 Outbound Hard-Bind

- 外部送信直前に `KQ_INPUT_PACKET_JSON` を再検証すること
- 欠落/不正時は **fail-close**（送信しない）

### 4.3 Exec Path Hard-Bind

- 実行系（shell/tool）に `KQ_INPUT_PACKET_JSON` を必ず伝搬すること
- 欠落/不正時は `rc=74` で拒否
