...
def main() -> int:
    ...
    steps.append(run([PY_BIN, "inf-Coding/inf-Coding-Assist/rust_host_diagnose.py"], cwd=ROOT))
    ...
    # 2) no-fallback green
    steps.append(run([PY_BIN, "inf-Coding/inf-Coding-Assist/rust_no_fallback_check.py"], cwd=ROOT, env=env_rust))

    # 3) parity green
    steps.append(run([PY_BIN, "inf-Coding/inf-Coding-Assist/rust_hotpath_parity_check.py"], cwd=ROOT, env=env_rust))

    # 4) rust migration coverage smoke (subset run in rust-only)
    env_smoke = env_rust.copy()
    env_smoke["IUT_STAGED_CHECK"] = "0"
    steps.append(run([PY_BIN, "inf-Coding-Assist/iut_subset_scaffold.py"], cwd=ROOT / "inf-Coding", env=env_smoke))

    # 5) strict-heavy compare (stdout-only)
    steps.append(run([PY_BIN, "inf-Coding/inf-Coding-Assist/strict_heavy_compare_stdout.py"], cwd=ROOT, env=env_smoke))
