...
P2_PATH = ROOT / 'P2_iut_handoff_20260307_v2.json'
P1_ROOT = ROOT / 'P1_artifacts_20260307_v3'
OUT_DIR = ROOT / 'P2_execution_20260307_v2'
...
def build_kernel_expr(handoff: dict[str, Any], kernel_name: str) -> str:
    profiles = handoff.get('mediation_profiles') or []
    unified = handoff.get('unified_fields') or []
    target = unified[0] if unified else 'unified_unknown'
    if kernel_name == 'hol_kernel':
        return f"forall bridge in {profiles}. preserves({target})"
    if kernel_name == 'smt_kernel':
        return f"vars: q>=0, r>=0; formula: bridge({target})"
    if kernel_name == 'uf_kernel':
        return f"eq: map({handoff['geom_ids'][0]})={target}, map({handoff['geom_ids'][1]})={target}"
    if kernel_name == 'ctl_kernel':
        return f"AG {target} @ [['{target}']]"
    return 'true'


def run_kernel(bridge: RustKQBridge, kernel_name: str, expr: str) -> dict[str, Any]:
    method = getattr(bridge, KERNEL_METHODS[kernel_name])
    try:
        return method(expr)
...
def execute_one(bridge: RustKQBridge, handoff: dict[str, Any]) -> dict[str, Any]:
    ...
    for kernel_name in handoff.get('proof_kernel_targets', []):
        if kernel_name not in KERNEL_METHODS:
            continue
        expr = build_kernel_expr(handoff, kernel_name)
        result = run_kernel(bridge, kernel_name, expr)
        kernel_results.append({'kernel': kernel_name, 'expr': expr, 'result': result, 'ok': bool(result.get('ok'))})
    mediation_pass = bool(handoff['compatibility_contract']['all_inputs_ready_for_p2'] and handoff['compatibility_contract']['profile_match_found'] and all(r['ok'] for r in kernel_results))
    return {
        'handoff_id': handoff['handoff_id'],
        'geom_ids': handoff['geom_ids'],
        'mediation_profiles': handoff['mediation_profiles'],
        'unified_fields': handoff['unified_fields'],
...
def main() -> int:
    ...
    summary = {
        'schema': 'inf-brain-p2-execution-summary-v2',
