"""
Katala_Samurai_31_b (KS31b) — Cyclic Three-Layer Verification System
...
class Layer1:
    """S01-S28 verification engine. The axis of KS31b."""
...
class KS31b:
    ...
    def verify(self, claim, store=None):
        ...
        if store:
            store.write("KS31b_R4_synthesis", synthesis)
        ...
        if store and step_meta:
            store.write("KS31b_R5_step_meta", step_meta)
    ...
    def _build_output(self, verdict, final_score, r1_result, trace, elapsed,
        ...
        if store:
            store.write("KS31b_final", output)
            store.finalize()
...
def run_tests():
    ...
    print("=" * 70)
    print(f"KS31b — Cyclic Three-Layer Verification")
    print("=" * 70)

    for label, claim in tests:
        with tempfile.TemporaryDirectory() as d:
            store = StageStore("ks31b_test", base_dir=d)
